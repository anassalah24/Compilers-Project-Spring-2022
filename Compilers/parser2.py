    ...
    def LR_parser(self, w):
        ...
        while True:
            ...
            if a not in self.parse_table[s]:
                ...
                nodedict[keyvalue]=str(a)
                tempdict[keyvalue]=str(a)
                keyvalue=keyvalue+1
                ...
                # 5azen el parent key value 3shan nwsalo lel children
                parent = keyvalue
                # e3ml list 7ot feha el values ely htkaren beha el body
                listofvalues = list(tempdict.values())
                listofkeys = list(tempdict.keys())

                keystobeconnected=[]
                # 7ot el parent ka node
                nodedict[keyvalue]=head
                tempdict[keyvalue]=head

                for i in body:

                   for  x in range(len(listofvalues)-1,-1,-1):

                       if str(i) == listofvalues[x]:
                           keystobeconnected.append(listofkeys[x])
                           listofvalues.reverse()
                           listofvalues[listofvalues.index(i)]=0
                           listofvalues.reverse()
                           del tempdict[listofkeys[x]]

                           break

                for i in keystobeconnected:
                   edgeslist.append((parent,i))
                keyvalue=keyvalue+1
...
def parseinput(sentence):
        ...
        while True:
            ...
            if (
                    tok.type == "ILLEGAL" or tok.type == "ILLEGALCOLON" or tok.type == "ILLEGALPLUS" or tok.type == "ILLEGALMINUS" or tok.type == "ILLEGAL_ID"):
                mytokens += "G"

        main(mytokens)


def drawparse(nodedict , edgelist ,nodelist):
    plt.close()
    G = nx.DiGraph()
    G.add_nodes_from(nodedict)
    G.add_edges_from(edgelist)
    pos = graphviz_layout(G, prog="dot")

    nx.draw_networkx(G,pos,nodelist=nodelist, node_color="white",with_labels=True,labels=nodedict , font_size=13,font_color='blue',arrows=False
                     )
    plt.axis('off')
...
#
#     main("if number then id := number ; end")